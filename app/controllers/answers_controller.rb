	...
	def generateCSV
		...
		if !tester_ids
			...
			tester_ids.each do |id|
				idResults = Answer.where(tester_id: id)
			...
			answer_csv = CSV.generate do |csv|
				csv << ["TestID", "Fråga", "Svar", "Svarsvärde", "Svarstid"]
				#loop
				data.each do |d|

					d.each do |r|
						question_label = Question.where(id: r.question).pluck(:text)
						csv << [r.tester_id, question_label[0], r.answer_label, r.answer_value, r.answer_time]
						#fest.push(r.tester_id)
					end
				end
			end
